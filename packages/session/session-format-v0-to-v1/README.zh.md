...
Alpha 迁移边只准入冻结清单之外、且信封带有 `ignorable: true` 的事件，并将其原样带入 v1；后续的重写边会省略该事件。其他未知类型仍会被拒绝。它也会拒绝意外的 payload 成员。`tool/result.meta` 与嵌套 PTC `arguments` 是显式的不透明 JSON 字段；迁移会原样保留它们，不把其中的数字解释为会话序号。内容块中未知的 `type` 分支、消息来源中未知的 `kind` 分支、assistant 结束原因中未知的 `kind` 分支与 `turn/end` 原因中未知的 `kind` 分支保持 owner-opaque JSON，已知分支则接受结构校验。
...
- **仅限 ignorable 的清单扩展**——只有当信封带有 `ignorable: true` 时，未知的外部插件事件才能跨过本边；没有该标记时，按当前 Alpha 策略会使迁移失败。

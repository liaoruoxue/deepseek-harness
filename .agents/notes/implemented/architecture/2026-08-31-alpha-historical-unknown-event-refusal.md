...
The alpha v0-to-v1 edge owns a frozen complete released-v0 event and payload inventory. It refuses every unknown historical event type before target staging, including an event marked `ignorable: true`, and refuses unexpected members of known payloads except fields explicitly classified as owner-opaque JSON. Merge-extensible nested discriminants remain part of that explicit policy: unknown content-block types, message-source kinds, assistant finish-reason kinds, and turn-ending reason kinds are preserved as owner-opaque JSON, while known arms receive structural validation. The [V2-to-V3 kind decision](2026-09-11-v2-to-v3-opaque-plugin-kinds.md) applies the same owner-opaque rule to that edge's own source and content kind sets. The diagnostic names the event type, its sequence number, and the unchanged source generation.

...
当前策略在保留精确源代际的同时省略显式可忽略的历史事件（[决策](2026-09-10-historical-migration-omits-ignorable-events.zh.md)）。显式外部 owner 迁移接口尚未实现。

`SessionSeq` 与 `SessionLogOffset` 让已知第一方数字字段可审计，但无法分类未知 runtime 对象中的数字。因此迁移规则不能根据没有识别到品牌字段来推断安全。

本记录仅在历史格式迁移方面取代 [保留可忽略外部 Session 事件](2026-08-30-retain-ignorable-external-session-events.zh.md)。原决定对同版本 append 与 reload 仍然有效。历史规则的措辞已经改变；[历史迁移省略显式可忽略的未知事件](2026-09-10-historical-migration-omits-ignorable-events.zh.md)负责带标记可忽略事件的省略，本记录对未标记拒绝与 payload 成员清单仍然有效。
